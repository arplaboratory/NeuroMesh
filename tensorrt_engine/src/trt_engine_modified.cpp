...
TRTEngine::TRTEngine(std::string model_filename, std::vector<std::vector<uint>> input_dims, int type_length, int batchSize){
...
void TRTEngine::runInference(const std::vector<const void*>& inputTensors, const std::vector<int>& inputSizes,
                    std::vector<void*>& outputTensors, const std::vector<int>& outputSizes) {
    if (inputTensors.size() + outputTensors.size() != bindings.size()) {
        throw std::runtime_error("Number of input and output tensors doesn't match engine bindings");
    }
...
//    std::cout << "IN ENGINE: my inputTensors.size() looks like " << inputTensors.size() << std::endl;
//    std::cout << "IN ENGINE: my inputSizes.size() looks like " << inputSizes.size() << std::endl;
//    std::cout << "IN ENGINE: my inputSizes[0] looks like " << inputSizes.at(0) << std::endl;
    // Copy input data to device
    for (size_t i = 0; i < inputTensors.size(); ++i) {
        cudaMemcpyAsync(bindings[i], inputTensors[i], inputSizes[i], cudaMemcpyHostToDevice, stream);
    }
    ...
    for (size_t i = 0; i < outputTensors.size(); ++i) {
        cudaMemcpyAsync(outputTensors[i], bindings[inputTensors.size() + i], outputSizes[i],
