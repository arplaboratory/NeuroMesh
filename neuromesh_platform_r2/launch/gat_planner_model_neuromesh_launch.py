...
def launch_setup(context):
    ...
    for i, agent in enumerate(complete_agent_list):
        ...
        remappings.append((f"/{name}/pos_{agent}", f"/{agent}/odom"))
        remappings.append(
            (f"/{name}/gnn_output_{agent}", f"/{agent}/gnn_output_{agent}")
        )

    remappings.append(("position_topic", f"/{name}/odom"))
    ...
    composable_nodes.append(
        ComposableNode(
            ...
            namespace=name,
            name=["neuromesh"],
            plugin="GATPlannerNeuromeshNode::GATPlannerImplementation",
            parameters=[
                {
                    "id": name,
...
def generate_launch_description():
    name_arg = DeclareLaunchArgument(
        name="name", default_value="voxl2_1", description=("Which agent we are running")
    )
    ...
    agent_num_arg = DeclareLaunchArgument(
        name="agent_num", default_value="4", description=("Which agent we are running")
    )

    agent_list_arg = DeclareLaunchArgument(
        name="agent_list",
        default_value="voxl2_1,voxl2_2,voxl2_5,voxl2_9",
