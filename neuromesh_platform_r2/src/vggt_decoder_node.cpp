...
VggtDecoderNode::VggtDecoderNode(const rclcpp::NodeOptions& options)
    ...
    this->declare_parameter<std::vector<std::string>>("vggt.robot_names", std::vector<std::string>{});
    ...
    decoder_cycle_interval_ = this->get_parameter("vggt.decoder.cycle_interval").as_double();
    feature_age_threshold_ = this->get_parameter("vggt.decoder.feature_age_threshold").as_double();
    num_robots_ = this->get_parameter("vggt.decoder.num_robots").as_int();
    decoder_model_path_ = this->get_parameter("vggt.decoder.model_path").as_string();
    ...
    decoder_model_name_ = (last_slash != std::string::npos) ? 
                          decoder_model_path_.substr(last_slash + 1) : decoder_model_path_;
    
    // Ensure current robot is in the robot list
    if (std::find(robot_names_.begin(), robot_names_.end(), robot_name_) == robot_names_.end()) {
    ...
    RCLCPP_INFO(this->get_logger(), "VggtDecoderNode initialized for robot: %s", robot_name_.c_str());
    RCLCPP_INFO(this->get_logger(), "Decoder model: %s", decoder_model_name_.c_str());
    RCLCPP_INFO(this->get_logger(), "Decoder cycle interval: %.2f seconds", decoder_cycle_interval_);
    RCLCPP_INFO(this->get_logger(), "Feature age threshold: %.2f seconds", feature_age_threshold_);
    RCLCPP_INFO(this->get_logger(), "Number of robots: %d", num_robots_);
    RCLCPP_INFO(this->get_logger(), "Robot names: %s", 
                std::accumulate(robot_names_.begin(), robot_names_.end(), std::string(),
    ...
    for (const auto& robot : robot_names_) {
        ...
        feature_subs_[robot] = this->create_subscription<neuromesh_interfaces::msg::Feature>(
            feature_topic, qos,
    ...
    pointcloud_pubs_["current"] = this->create_publisher<sensor_msgs::msg::PointCloud2>(pc_current_topic, 10);
    pointcloud_pubs_["neighbor"] = this->create_publisher<sensor_msgs::msg::PointCloud2>(pc_neighbor_topic, 10);
    rgb_pointcloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(pc_rgb_topic, 10);
...
void VggtDecoderNode::feature_callback(const neuromesh_interfaces::msg::Feature::SharedPtr msg) {
    std::lock_guard<std::mutex> lock(feature_mutex_);
    
    RCLCPP_DEBUG(this->get_logger(), "Received features from robot: %s", msg->id.c_str());
...
void VggtDecoderNode::decoder_timer_callback() {
    ...
    if (processing_in_progress_.load()) {
        RCLCPP_WARN(this->get_logger(), "Previous decoding still in progress, skipping this cycle");
        return;
    }
    
    // Check if decoder result is ready
    if (decoder_result_.valid() && 
        decoder_result_.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
        
        RCLCPP_INFO(this->get_logger(), "Decoder result ready, processing outputs...");
        try {
            ...
            RCLCPP_ERROR(this->get_logger(), "Error processing decoder result: %s", e.what());
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lock(feature_mutex_);
        has_self_features = feature_buffer_.find(robot_name_) != feature_buffer_.end();
    }
    
    if (!has_self_features) {
        RCLCPP_DEBUG(this->get_logger(), "No features from self (%s), skipping decoder", robot_name_.c_str());
        return;
    }
    ...
    if (build_decoder_tensor(decoder_input)) {
        RCLCPP_INFO(this->get_logger(), "Starting decoder inference with tensor size: %zu bytes", 
...
bool VggtDecoderNode::build_decoder_tensor(neuromesh_interfaces::msg::Tensor& output_tensor) {
    std::lock_guard<std::mutex> lock(feature_mutex_);
    
    // Get self features (required)
    if (feature_buffer_.find(robot_name_) == feature_buffer_.end()) {
        RCLCPP_ERROR(this->get_logger(), "No features available from self (%s)", robot_name_.c_str());
        return false;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Building decoder tensor for %d robots", num_robots_);
    ...
    for (int i = 0; i < num_robots_; ++i) {
            ...
            if (feature_buffer_.find(target_robot) != feature_buffer_.end()) {
                ...
                if (check_feature_freshness(target_robot, age_seconds)) {
                    ...
                    RCLCPP_WARN(this->get_logger(), 
                               ...
                               target_robot.c_str(), age_seconds, feature_age_threshold_);
                    feature_data = reinterpret_cast<const float*>(self_features->tensor.data.data());
                }
            } else {
        ...
        // Copy feature data
        aggregated_data.insert(aggregated_data.end(), feature_data, feature_data + feature_size);
        RCLCPP_DEBUG(this->get_logger(), "Robot %d: using features from %s", i, source_robot.c_str());
    }
    
    // Build output tensor
    output_tensor.name = "aggregated_features";
    output_tensor.shape.dims = {static_cast<uint32_t>(num_robots_), 1036, 1024};  // VGGT-specific dimensions
    ...
    std::memcpy(output_tensor.data.data(), aggregated_data.data(), output_tensor.data.size());
    
    RCLCPP_INFO(this->get_logger(), "Built decoder tensor with shape [%d, 1036, 1024], size: %zu bytes",
                num_robots_, output_tensor.data.size());
    
    return true;
}

bool VggtDecoderNode::check_feature_freshness(const std::string& robot_name, double& age_seconds) {
...
void VggtDecoderNode::process_decoder_output(
    const std::vector<std::shared_ptr<neuromesh_interfaces::msg::Tensor>>& outputs) {
    ...
    RCLCPP_INFO(this->get_logger(), "Processing %zu decoder outputs", outputs.size());
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        RCLCPP_DEBUG(this->get_logger(), "Output %zu: %s, shape: [%s]", i, tensor->name.c_str(),
        ...
        if (tensor->name == "depth" || i == 1) {
            ...
            world_points_conf_data = reinterpret_cast<const float*>(tensor->data.data());
        }
    }
    ...
    for (int robot_idx = 0; robot_idx < num_robots_; ++robot_idx) {
        ...
        if (world_points_data && world_points_conf_data) {
            // Calculate offsets for this robot's data
            size_t pixel_count = depth_width_ * depth_height_;
            const float* robot_points = world_points_data + robot_idx * pixel_count * 3;
            const float* robot_points_conf = world_points_conf_data + robot_idx * pixel_count;
            
            // Create and publish point cloud
            std::string frame_id = robot_name_ + "_base_link";
            auto pointcloud = create_point_cloud(robot_points, robot_points_conf,
                                               depth_width_, depth_height_, frame_id);
            
            // Publish to appropriate topic
            if (robot_idx == 0 && pointcloud_pubs_.find("current") != pointcloud_pubs_.end()) {
                pointcloud_pubs_["current"]->publish(pointcloud);
            } else if (robot_idx == 1 && pointcloud_pubs_.find("neighbor") != pointcloud_pubs_.end()) {
                pointcloud_pubs_["neighbor"]->publish(pointcloud);
            }
...
sensor_msgs::msg::Image VggtDecoderNode::create_depth_image(
    const float* depth_data, const float* confidence_data,
...
sensor_msgs::msg::PointCloud2 VggtDecoderNode::create_point_cloud(
