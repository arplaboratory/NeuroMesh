...
GATneuromeshNode :: GATneuromeshNode(const rclcpp::NodeOptions &options): Node("GAT_neuromesh_node", options)
{
    ...
    // Initialize GNN result publisher and subscriber
	...
	decoder_timer_ = this->create_wall_timer(std::chrono::duration<int,std::milli>(decoder_cycle_length_), std::bind(&GATneuromeshNode::run_decoder_cycle, this));
	encoder_timer_ = this->create_wall_timer(std::chrono::duration<int,std::milli>(encoder_cycle_length_), std::bind(&GATneuromeshNode::run_encoder_cycle, this));
...
void GATneuromeshNode::load_goal_poses_from_yaml() {
    try {
...
neuromesh_interfaces::msg::Tensor GATneuromeshNode::calculate_goal_distances(const std::vector<float>& state_vector) {
...
neuromesh_interfaces::msg::Feature GATneuromeshNode::buildFeatureMessage(const neuromesh_interfaces::msg::Tensor& tensor)
{
...
void GATneuromeshNode::feature_callback(const neuromesh_interfaces::msg::Feature::SharedPtr msg) {
...
//PLACEHOLDER
std::future<std::vector<std::shared_ptr<neuromesh_interfaces::msg::Tensor>>> GATneuromeshNode::performInference(const std::string& model_name, const std::vector<neuromesh_interfaces::msg::Tensor>& tensors)
{
...
//PLACEHOLDER
bool GATneuromeshNode::buildDecoderTensor(
...
void GATneuromeshNode::createSubscription(std::map<std::string, rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr>& subscription_map, std::string id, rclcpp::QoS qos)
{
	...
	rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr feature_subscription_ = 
		this->create_subscription<neuromesh_interfaces::msg::Feature>(
			topic,
			qos,
			std::bind(&GATneuromeshNode::feature_callback, this, std::placeholders::_1));

	subscription_map.insert( {id, feature_subscription_} );
}

//Remove subscription form the feature_subscriptions_ map
void GATneuromeshNode::removeSubscription(std::map<std::string, rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr> subscription_map, std::string id)
{
	subscription_map.erase(id);
}

//Adds a subscription to the gnn_subscriptions_ map
void GATneuromeshNode::createGNNSubscription(std::map<std::string, rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr>& gnn_subscription_map, std::string id, rclcpp::QoS qos)
{
	...
	rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr gnn_subscription_ = 
		this->create_subscription<neuromesh_interfaces::msg::Feature>(
			topic,
			qos,
			std::bind(&GATneuromeshNode::gnn_result_callback, this, std::placeholders::_1));

	gnn_subscription_map.insert( {id, gnn_subscription_} );
}

//Remove subscription form the gnn_subscriptions_ map
void GATneuromeshNode::removeGNNSubscription(std::map<std::string, rclcpp::Subscription<neuromesh_interfaces::msg::Feature>::SharedPtr> gnn_subscription_map, std::string id)
{
...
rmw_qos_profile_t GATneuromeshNode::parseQoSString(const std::string& str)
{
...
std::set<std::string> GATneuromeshNode::splitAgentString(std::string str)
{
...
void GATneuromeshNode::run_encoder_cycle() {
    ...
    if (fresh_encoder_cycle && !current_states_.empty() && pos_callback_complete && !goal_poses_.empty() && !waypoint_cmd_sent_) {
        ...
        neuromesh_interfaces::msg::Tensor goal_distances = calculate_goal_distances(
        ...
        encoder_result = performInference(encoder_model_name_, {goal_distances});
...
void GATneuromeshNode::run_decoder_cycle() {
...
neuromesh_interfaces::msg::Feature GATneuromeshNode::build_gnn_msg(const neuromesh_interfaces::msg::Tensor& gnn_result) {
...
void GATneuromeshNode::gnn_result_callback(const neuromesh_interfaces::msg::Feature::SharedPtr msg) {
...
void GATneuromeshNode::prepare_second_stage_decoding() {
...
void GATneuromeshNode::startClock(std::string phase){
...
void GATneuromeshNode::stopClock(std::string phase){
...
int64_t GATneuromeshNode::checkClock(std::string phase){
	if (times[phase].second){
...
neuromesh_interfaces::msg::Tensor GATneuromeshNode::convert_to_nchw(const neuromesh_interfaces::msg::Tensor& input){
